...
<body>
    <div id="app">
        ...
        <div id="firm-panel">
            <div class="panel-header">
                <div class="close-panel" onclick="closeFirmPanel()">✕</div>
    ...
    <script>
        ...
        } else if (storedFirms.length > 0) {
            firms = storedFirms;
        } else {
            firms = generateSampleData();
            persistFirms('sample', firms);
        }
        ...
        function generateSampleData() {
            const firmNames = [
                'Atlas Capital Partners', 'Vertex Ventures', 'Summit Partners', 'Silver Lake',
                'Thoma Bravo', 'Vista Equity', 'Insight Partners', 'General Atlantic',
                'KKR', 'Blackstone', 'Apollo', 'Carlyle Group', 'Warburg Pincus',
                'TPG Capital', 'Advent International', 'CVC Capital', 'Permira',
                'Bain Capital', 'Providence Equity', 'Hellman & Friedman'
            ];

            const cities = Object.keys(CITY_COORDS);

            return firmNames.map((name, i) => {
                const city = cities[Math.floor(Math.random() * cities.length)];
                return withBasePosition({
                    id: 'firm_' + Date.now() + '_' + i,
                    name: name,
                    aum: Math.random() * 100 + 1,
                    stage: Math.floor(Math.random() * STAGE_COUNT) + 1,
        ...
        function generatePeople() {
            const roles = ['GP', 'MD', 'Principal', 'VP', 'Associate'];
